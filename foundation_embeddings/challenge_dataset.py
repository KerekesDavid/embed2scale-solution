    ...
    def __getitem__(self, idx):
        ...
        if self.randomize_seasons:
            seasons = [
                ...
                for ind in torch.randperm(len(self.possible_seasons)).tolist()[
                    : self.seasons
                ]
            ]
        else:
        ...
        for modality, sample_path in zip(self.modalities, sample_paths):
            ...
            data[modality] = (
                xr.open_zarr(sample_path)
                .isel(time=season_index)[self.dataset_name]
                .values
            )
        ...
        start_ind_of_modality = {
            ...
            for m, n in zip(
                self.modalities,
                [0] + np.cumsum(list(n_bands_per_modality.values())).tolist(),
            )
        }
        ...
        if not self.concat:
            data = {
                m: data[
                    ...,
                    start_ind_of_modality[m] : start_ind_of_modality[m]
                    + n_bands_per_modality[m],
                    :,
                    :,
                ]
